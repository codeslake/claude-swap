...
def proper_lockfile(
    ...
    *,
    ...
    # reaches three times.
    # THE LATCH IS PER FREEZE, NOT PER HOLD. A refresh that lands ends the
    # episode the warning describes, so the next freeze that outlives
    # `staleness` is a new fact and the takeover it precedes is explained.
    warned = False
    last_ok = time.time()
    adopt_stamp = False
    ...
    def _warn_frozen(e: OSError) -> None:
        """Say ONCE that the mtime has stopped advancing, and only once the
        sentence is true -- a freeze that has outlived `staleness`. Firing on
        the first failure announced imminent theft over a lock that stayed
        fresh."""
        nonlocal warned
    ...
    def _touch() -> None:
        ...
        while not stop_touching.wait(TOUCH_INTERVAL_S):
            with _tick_guard:
                ...
                # THIS TICK LANDED, so the freeze episode is over.
