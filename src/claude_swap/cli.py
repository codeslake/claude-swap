...
Examples:
  ...
  cswap pin --clear          # remove the pin
    ...
    try:
        ...
        # Whether THIS pin is the one that turns the credential on. Say it
        # without a count: the number cannot be measured. Proxy env is
        # inherited by every descendant, so walking /proc for the variable
        # counts `sleep`, `dbus-daemon` and 18 gnome-keyring-daemons — 211 on
        # this box against 1 real session. And /proc/<pid>/environ is an
        # EXEC-TIME snapshot, while Claude Code applies its .claude.json env
        # block at boot: measured, this very session's environ says
        # HTTPS_PROXY=:9901 while its sockets go to :36301. So environ can
        ...
        if _will_arm:
            print(
                dimmed(
                    "The pin proxy now requires a credential. Sessions already "
                    "running hold the old address and cannot pick it up in "
                    "place; they fail with 407 until relaunched. `cc-update "
                    "--apply --force` relaunches them where they are."
                )
            )
        print(
