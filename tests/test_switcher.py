...
class TestActiveAccountRefresh:
    ...
    def test_lineage_lookup_failure_after_post_still_reaches_the_stores(
        self, temp_home: Path, mock_claude_config: Path, sample_sequence_data: dict
    ):
        """A torn `sequence.json` mid-flight makes `account_identity` (via
        `_lineage_key`'s self-attributed-verdict lookup) raise -- after the
        grant is already consumed. The memo lookup is best-effort only:
        neither store read below it depends on the roster, so the successor
        must still reach the slot backup and the live store, not be stashed
        and deferred (which would cost a pass for nothing and, worse, is a
        NO-OP on a slot the both-fail arm never reaches)."""
        switcher = self._switcher(sample_sequence_data)
    ...
    def test_stashed_active_successor_is_adopted_without_a_post(
        self, temp_home: Path, mock_claude_config: Path, sample_sequence_data: dict
    ):
        """A prior pass POSTed the backup's refresh token and could not
        persist the successor anywhere durable, parking it in
        `self._unpersisted`. The next pass must adopt it before choosing a
        refresh_input, not re-POST the now-spent backup token."""
        switcher = self._switcher(sample_sequence_data)
        switcher._write_account_credentials("1", "test@example.com", self._EXPIRED)
    ...
    def test_live_keyed_adopt_recovers_a_stash_entry_after_a_restart(
        self, temp_home: Path, mock_claude_config: Path, sample_sequence_data: dict
    ):
        """`_probe_verdicts` lives only in memory: a process restart between
        the both-fail pass that stashed a live-sourced successor (see
        `test_live_sourced_stash_successor_is_adopted_without_a_repost`
        above) and the next pass leaves a FRESH switcher with no verdict for
        live's lineage -- even though the on-disk stash entry itself
        (`configSlot` == this slot, `consumedFp` == fp(live)) is a record of
        this tool's own prior POST. That entry must license the adopt too,
        or Claude Code POSTs the already-spent live grant on its next use."""
        live_newer = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-newer", "refreshToken": "rt-newer",
                "expiresAt": 2000,
        ...
        switcher = self._switcher(sample_sequence_data)  # fresh: no verdicts
        switcher._write_account_credentials("1", "test@example.com", self._EXPIRED)
        switcher._store._write_unclaimed_credential(
            self._REFRESHED,
            {
        ...
        assert switcher._probe_verdicts == {}, "premise: no in-memory verdict"
        ...
        mock_refresh.assert_not_called()      # adopted, not re-POSTed
        assert switcher._read_account_credentials(
            "1", "test@example.com"
        ) == self._REFRESHED
        write_live.assert_called_once_with(self._REFRESHED)
    ...
    def test_live_keyed_adopt_ignores_a_stash_entry_for_another_slot(
        ...
        """Control for the restart-recovery adopt above: a stash entry whose
        `configSlot` names a DIFFERENT slot must not license anything, even
        though its `consumedFp` matches live's fingerprint -- the
        unattributed arm still defers to Claude Code, no POST either."""
        live_newer = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-newer", "refreshToken": "rt-newer",
                "expiresAt": 2000,
        ...
        assert len(switcher.list_unclaimed_credentials()) == 1  # survives

    def test_live_keyed_stash_unreadable_surfaces_stash_unreadable(
        self, temp_home: Path, mock_claude_config: Path,
        sample_sequence_data: dict, monkeypatch,
    ):
        """The live-keyed adopt call's own except arm must surface an
        unreadable stash entry as `stash-unreadable`, the same remedy the
        backup-keyed call already gets
        (`test_unreadable_stash_manifest_surfaces_stash_unreadable_not_expired`
        above) -- not `_defer`'s generic sentinel."""
        live_newer = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-newer", "refreshToken": "rt-newer",
                "expiresAt": 2000,
        ...
        def _deny(path, *a, **kw):
            if path.name == f".unclaimed-{entry_id}.enc":
                raise PermissionError(13, "Permission denied")
    ...
    def test_live_keyed_stash_write_failure_surfaces_stash_write_failed(
        self, temp_home: Path, mock_claude_config: Path, sample_sequence_data: dict
    ):
        """Distinct from the read failure above: the in-memory live-keyed
        successor IS readable, it just cannot be written back -- must
        surface as `stash-write-failed`, mirroring the backup-keyed except
        arm's own row
        (`test_in_memory_successor_write_failure_surfaces_stash_write_failed`
        above)."""
        live_newer = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-newer", "refreshToken": "rt-newer",
    ...
    def test_stash_pending_refresh_logs_before_a_second_interrupt_propagates(
        self, temp_home: Path, mock_claude_config: Path,
        sample_sequence_data: dict, caplog,
    ):
        """Mirrors `_consume_backup_grant_locked`'s own second-interrupt arm
        (~2796): a Ctrl-C landing while THIS stash write is itself blocked
        must be logged, not vanish silently, before the interrupt reaches
        the caller."""
        import logging
        switcher = self._switcher(sample_sequence_data)
        ...
        with patch.object(
            ...
            with pytest.raises(KeyboardInterrupt):
                switcher._stash_pending_refresh("1", self._REFRESHED, "fp-old")
...
class TestProbeTargetCredentialReprobe:
    """`_probe_target_credential`'s escalation re-probes whatever
    `consume_backup_grant` hands back before trusting it live or dead."""
    ...
    def test_unchanged_reprobe_token_is_not_struck(
        self, temp_home: Path, sample_sequence_data: dict
    ):
        """The escalation answered success (`error is None`) but its
        credentials carry the SAME access token that just answered the
        real 401 (a malformed refresh response with no usable
        access_token, or a lineage already rotated elsewhere) -- re-probing
        it proves nothing about whether the REFRESH GRANT is dead, only
        that the stale token still doesn't work, which the first probe
        already established. Must not strike."""
        s = self._switcher(sample_sequence_data)
        creds = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-stale", "refreshToken": "rt-old",
                "expiresAt": 9999999999000,
            }
        })
        # Same access token, but escalation-shaped bytes (e.g. a fresh
        # refreshToken) -- only accessToken has to match for the guard.
        unchanged = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-stale", "refreshToken": "rt-new",
                "expiresAt": 9999999999000,
            }
        })
        ...
        with patch(
            ...
            s, "consume_backup_grant",
            return_value=oauth.RefreshOutcome(unchanged, None),
        ), patch.object(s, "_strike_dead_target") as mock_strike:
            live, creds_out, proven_401, stash_state = (
                s._probe_target_credential("1", "test@example.com", creds)
            )

        assert mock_probe.call_count == 2, "must still re-probe once"
        mock_strike.assert_not_called()
        assert live is None
        assert proven_401 is True

    def test_changed_reprobe_token_still_dead_is_struck(
        self, temp_home: Path, sample_sequence_data: dict
    ):
        """CONTROL: a genuinely NEW access token that also answers dead is
        real evidence -- the strike must still fire."""
        s = self._switcher(sample_sequence_data)
        creds = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-stale", "refreshToken": "rt-old",
        ...
        rotated = json.dumps({
            "claudeAiOauth": {
                "accessToken": "sk-fresh", "refreshToken": "rt-new",
                "expiresAt": 9999999999000,
            }
        })
        ...
        mock_strike.assert_called_once_with(
            "1", "test@example.com", oauth.credential_fingerprint(rotated)
        )
...
class TestRefusedAdoptionStaysReadOnly:
    ...
    def test_control_readable_quiescent_record_still_adopts(
        ...
        seen = self._run(
            ...
            # pid 1 is deterministically dead (is_pid_alive returns False for
            # pid <= 1); an arbitrary large pid is a legal live pid on a busy
            # host and also reads alive on PermissionError, so it is not a
            # reliable "definitely not running" fixture.
            json.dumps({"pid": 1}),
        )
        assert seen["creds"] == self.FRESH_PROFILE
...
class TestPermanentlyUnreadableStashRow:
    ...
    def test_in_memory_successor_write_failure_reports_its_own_reason(
        self, temp_home: Path, sample_sequence_data: dict
    ):
        """A write-back failure for an IN-MEMORY stashed successor (see
        `_adopt_stashed_successor`'s `self._unpersisted` branch) is a
        storage WRITE fault, not a read one — the bytes are sitting right
        there in memory. It must report its own kind, not `stash-unreadable`
        (whose remedy, "unlock the keychain or fix the file", is the wrong
        advice for a write failure)."""
        s = self._switcher(sample_sequence_data)
        s._write_account_credentials("1", "test@example.com", self._OLD)
        s._unpersisted["1"] = (
            oauth.credential_fingerprint(self._OLD), self._NEW
        )

        with patch.object(
            s, "_write_account_credentials", side_effect=OSError("disk full")
        ), patch("claude_swap.oauth.try_refresh_oauth_credentials") as post:
            out = s.consume_backup_grant("1", "test@example.com", self._OLD)

        assert not post.called, "the spent generation must never be re-POSTed"
        assert out.error == "stash-write-failed", out.error
        assert "1" in s._unpersisted, (
            "the write failed -- the in-memory successor must stay parked"
        )
...
class TestGateUltraReviewFixes:
    ...
    def test_placeholder_survives_when_the_reasoned_stash_also_fails(
        self, temp_home: Path, sample_sequence_data: dict, monkeypatch, caplog
    ):
        """The reasoned stash write (after a normal-persist failure) can
        itself fail — but the placeholder written right after the POST,
        before the slot lock, already holds the successor durably. Report
        `stashed=True` (not the unrecoverable `consume-gate-unpersisted`
        shape session.py reads as "the successor is gone") and do not log
        "it is lost"."""
        import logging
        s = self._switcher(sample_sequence_data)
        s._write_account_credentials("1", "test@example.com", self._OLD)
        ...
        def flaky_stash(creds, ctx):
            state["stash_calls"] += 1
            if state["stash_calls"] == 1:
                # The placeholder, written right after the POST — let it land.
        ...
        monkeypatch.setattr(s._store, "_write_unclaimed_credential", flaky_stash)
        ...
        assert out.stashed is True, (
            "the placeholder already holds the successor -- must not "
            "report stashed=False"
        )
        entries = s.list_unclaimed_credentials()
        assert entries, "the placeholder must survive on disk"
        (entry,) = entries.values()
        assert entry["reason"] == "consume-gate-post-window"
        assert "1" not in s._unpersisted, (
            "no need for the in-memory fallback -- the placeholder already "
            "covers it"
        )
        assert not any("it is lost" in r.message for r in caplog.records)
    ...
    def test_interrupt_during_the_placeholder_write_still_stashes(
        self, temp_home: Path, sample_sequence_data: dict, monkeypatch
    ):
        """The placeholder write (`consume-gate-post-window`) taken right
        after the POST, before the slot lock, is now INSIDE the outer try —
        a KeyboardInterrupt racing that write itself must be caught by the
        `except BaseException` arm below it and stashed via the reasoned
        `stash_successor` call, not escape uncaught (which would lose the
        successor outright, since nothing else has stashed anything yet)."""
        s = self._switcher(sample_sequence_data)
        s._write_account_credentials("1", "test@example.com", self._OLD)
        ...
        def flaky_stash(creds, ctx):
            state["calls"] += 1
            if state["calls"] == 1:
                raise KeyboardInterrupt("during the placeholder write")
            return real_stash(creds, ctx)
        ...
        entries = s.list_unclaimed_credentials()
        assert entries, (
            "an interrupt during the placeholder write itself must not "
            "lose the successor"
        )
        (entry,) = entries.values()
        assert entry["reason"] == "consume-gate-interrupted"
    ...
    def test_second_interrupt_during_the_stash_attempt_still_reraises_the_first(
        ...
        interrupt escapes straight past its own log AND past the outer
        `raise`, so the ORIGINAL interrupt is replaced by the second one —
        losing which interrupt reached the caller and losing the log both.
        The placeholder written right after the POST DID land here (the
        test's own setup lets that first stash call through), so the log
        must say the successor is safe, not "it is lost"."""
        import logging
        s = self._switcher(sample_sequence_data)
        s._write_account_credentials("1", "test@example.com", self._OLD)
...
class TestT1313KeepsTheActiveAccountLive:
    """A `/login` for a DIFFERENT slot (N) is enrolment, never a switch --
    the sibling class already pins that for `activeAccountNumber`. T1313
    closes the axis that invariant left open: the live STORE and
    ``~/.claude.json`` themselves must not be left serving Account-N's
    login on behalf of the roster's own active slot (A), when A's own
    backup is right there, healthy and unquarantined.

    Slot 1 (``c@example.com``) is N, the account the server says the live
    credential belongs to. Slot 2 (``b@example.com``) is A, the roster's
    active slot, whose OWN backup + config are seeded so the restore has
    something to put back.
    """

    def _setup(
        self, sample_sequence_data, temp_home, *, n_disabled: bool = False,
    ):
        accs = sample_sequence_data["accounts"]
    ...
    def _assert_active_slot_untouched(
        ...
        assert oauth.credential_fingerprint(live_now) == oauth.credential_fingerprint(
            ...
            "DEFECT: the live store was left serving the login adopted into "
            "a DIFFERENT slot instead of the roster's own active account"
        )
    ...
    def test_manual_collect_path(
        ...
        with patch("claude_swap.oauth.fetch_oauth_profile",
                   return_value={"uuid": "u-1", "email": "c@example.com",
                                 "organizationUuid": "o-1"}):
            s._resync_rotated_backup("2", "b@example.com", "o-2", login)
    ...
    def test_n_equals_a_updates_the_backup_and_leaves_live_alone(
        ...
        assert json.loads(s._read_account_credentials(
            ...
            "the active slot's own backup was not resynced to its login"
        )
        live_now = s._read_credentials()
        assert oauth.credential_fingerprint(live_now) == oauth.credential_fingerprint(
            login
...
class TestLoginLineHelper:
    """`_log_detected_login`: every detected login gets exactly one INFO
    ``login:`` line per (fingerprint, outcome), whatever the outcome, so a
    masked login is never silent again -- deduplicated across repeats (the
    TUI's 3s tick must not spam an unchanged verdict) and never carrying a
    token."""

    _CREDS = json.dumps({"claudeAiOauth": {
        "accessToken": "sk-ant-oat01-SECRET-ACCESS-VALUE",
        "refreshToken": "sk-ant-ort01-SECRET-REFRESH-VALUE",
        "expiresAt": 1}})
    ...
    def test_one_line_per_fp_outcome_across_two_passes(
        self, temp_home: Path, caplog, outcome: str,
    ):
        import logging

        s = ClaudeAccountSwitcher()
        with caplog.at_level(logging.INFO, logger="claude-swap"):
            s._store._log_detected_login(self._CREDS, slot="1", outcome=outcome)
            s._store._log_detected_login(self._CREDS, slot="1", outcome=outcome)
        lines = [
            r.getMessage() for r in caplog.records
            if r.getMessage().startswith("login:")
        ]
        assert len(lines) == 1, f"expected one deduped line, got {lines}"
        assert outcome in lines[0]
        assert "SECRET-ACCESS-VALUE" not in lines[0]
        assert "SECRET-REFRESH-VALUE" not in lines[0]

    def test_a_changed_outcome_for_the_same_login_logs_again(
        self, temp_home: Path, caplog,
    ):
        import logging
        ...
        with caplog.at_level(logging.INFO, logger="claude-swap"):
            s._store._log_detected_login(
                self._CREDS, slot=None, outcome="ignored: cooldown",
            )
            s._store._log_detected_login(
                self._CREDS, slot=None, outcome="refused: probe failed",
            )
        lines = [
            r.getMessage() for r in caplog.records
            if r.getMessage().startswith("login:")
    ...
    def test_the_no_drift_return_never_logs(
        ...
        """The steady state -- the live credential already agrees with the
        matched slot's own backup -- is not a login."""
        import logging
        ...
        s._write_json(s.sequence_file, sample_sequence_data)
        same = json.dumps({"claudeAiOauth": {
            "accessToken": "sk-a", "refreshToken": "sk-r",
            "expiresAt": 99_999_999_999_999}})
        s._write_account_credentials("1", "account1@example.com", same)
        with caplog.at_level(logging.INFO, logger="claude-swap"):
            s._resync_rotated_backup("1", "account1@example.com", "", same)
        assert not any(
...
class TestCrossLineageFreshnessIsWriteTimeNotRefreshLifetime:
    """T1312 root cause: the file-vs-Keychain freshness arbitration
    (``credentials._fresher_plaintext_login``) ordered two DIFFERENT logins
    by ``refreshTokenExpiresAt``, which the server does not order across
    accounts (measured: a login at 00:55:59 carried an earlier stamp than
    one made at 00:54:28). A plaintext file left holding another account's
    bytes -- an earlier WRITE, but a later ``refreshTokenExpiresAt`` --
    masked a genuinely fresher Keychain login of the active account for
    every collect pass: the resync read the masked bytes, saw no drift from
    the active slot's own stale backup, and wrote nothing.
    """

    def test_a_collect_pass_writes_the_active_slot_from_the_fresher_keychain_login(
        self, temp_home: Path, monkeypatch, block_real_keychain,
    ):
        s = ClaudeAccountSwitcher()
        s.platform = Platform.MACOS
        ...
        s._write_json(s.sequence_file, {
            ...
            "sequence": [1],
            "accounts": {
                "1": {"email": "n@example.com", "uuid": "uuid-n",
                      "organizationUuid": "org-n"},
            },
        })
        s._write_account_credentials("1", "n@example.com", json.dumps({
            "claudeAiOauth": {"accessToken": "sk-n-old", "refreshToken": "rt-n-old",
                              "expiresAt": 1_000}}))
        (temp_home / ".claude.json").write_text(json.dumps({
            "oauthAccount": {"emailAddress": "n@example.com",
                             "accountUuid": "uuid-n",
                             "organizationUuid": "org-n"},
        }))

        # The Keychain: Account-1's fresh login, written just now -- but its
        # plan's refreshTokenExpiresAt is EARLIER than the stale file below.
        kc_creds = json.dumps({"claudeAiOauth": {
            "accessToken": "sk-n-fresh", "refreshToken": "rt-n-fresh",
            "expiresAt": 99_999_999_999_999, "refreshTokenExpiresAt": 1_000}})
        # Seed the SAME in-memory fake `_write_account_credentials` above and
        # the resync's own backup read/write already use (`block_real_keychain`,
        # autouse) rather than overriding `get_password` wholesale, which
        # would also blind the backup-service reads to what gets written.
        ...
        monkeypatch.setattr(
            ...
            lambda service, account: time.time(),
        )

        # The plaintext file: leftover bytes from a DIFFERENT account -- a
        # write from an hour ago, but a LATER refreshTokenExpiresAt.
        other_creds = json.dumps({"claudeAiOauth": {
            "accessToken": "sk-other", "refreshToken": "rt-other",
            "expiresAt": 99_999_999_999_999, "refreshTokenExpiresAt": 9_000}})
        cred_file = temp_home / ".claude" / ".credentials.json"
        cred_file.write_text(other_creds)
        stale = time.time() - 3600
        os.utime(cred_file, (stale, stale))

        with patch("claude_swap.oauth.fetch_oauth_profile",
                   return_value={"uuid": "uuid-n", "email": "n@example.com",
                                 "organizationUuid": "org-n"}):
            s.usage_entries_by_account(fetch=set())

        got = s._read_account_credentials("1", "n@example.com")
        assert got, "Account-1's backup went missing"
        assert json.loads(got)["claudeAiOauth"]["refreshToken"] == "rt-n-fresh", (
            "DEFECT: the plaintext file's other-account bytes, ordered "
            "'fresher' by refreshTokenExpiresAt alone, masked the active "
            "account's own Keychain login and the collect pass wrote nothing"
        )
