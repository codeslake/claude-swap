...
class TestAHealthySlotRetiresWhatItSuperseded:
    """The mirror of the adopt, and the half that was missing.

    A `foreign` stash is never adopted by `_adopt_stashed_successor` — that
    one matches on `consumedFp`, which an ownership verdict does not carry —
    so nothing ever dropped one. Measured across three machines: 17 entries,
    every one a DISTINCT credential, about one a day, the oldest five days
    old and still holding a live refresh token.

    SUPERSESSION IS THE CRITERION, not age and not expiry. On the same 17:
    an expiry rule reclaimed 2, because a refresh token outlives the stash by
    weeks; supersession reclaimed every one it was asked about, because each
    owning slot had long since moved to different bytes.
    """

    def _switcher(self, sample_sequence_data, live):
        sw = ClaudeAccountSwitcher()
        sw._setup_directories()
        sample_sequence_data["accounts"]["2"]["email"] = "owner@example.com"
        sample_sequence_data["accounts"]["2"]["uuid"] = "uuid-owner"
        sw._write_json(sw.sequence_file, sample_sequence_data)
        sw._write_account_credentials("2", "owner@example.com", live)
        return sw
    ...
    def test_CONTROL_the_bytes_the_slot_still_holds_are_kept(
        self, temp_home, mock_claude_config, sample_sequence_data
    ):
        """THE ONE THAT MATTERS. Retiring here would drop the copy of exactly
        what the slot is authenticating with."""
        sw = self._switcher(sample_sequence_data, FRESH)
        eid = self._stash(sw, FRESH)
        assert sw._retire_superseded_stash_for_slot("2", "owner@example.com") == 0
        assert eid in sw._store._list_unclaimed_credentials()

    def test_CONTROL_a_successor_stash_is_never_touched(
        self, temp_home, mock_claude_config, sample_sequence_data
    ):
        """Keyed by `consumedFp` and its own adopter still needs it. Only an
        ownership verdict is this method's business."""
        sw = self._switcher(sample_sequence_data, FRESH)
        eid = self._stash(sw, DEAD, reason="successor", consumedFp="whatever")
        assert sw._retire_superseded_stash_for_slot("2", "owner@example.com") == 0
        assert eid in sw._store._list_unclaimed_credentials()

    def test_CONTROL_another_slots_stash_is_never_touched(
        self, temp_home, mock_claude_config, sample_sequence_data
    ):
        """The identity gate, same as the adopt's: a different uuid is a
        different account and this slot may say nothing about it."""
        sw = self._switcher(sample_sequence_data, FRESH)
        eid = self._stash(sw, DEAD, resolvedIdentity={
            "uuid": "uuid-somebody-else", "email": "other@example.com",
            "organizationUuid": None})
        assert sw._retire_superseded_stash_for_slot("2", "owner@example.com") == 0
        assert eid in sw._store._list_unclaimed_credentials()

    def test_a_stash_the_slot_has_moved_past_is_dropped(
        self, temp_home, mock_claude_config, sample_sequence_data
    ):
        """The population the method exists for: the slot authenticates with
        FRESH, so the parked DEAD bytes can only move it backwards."""
        sw = self._switcher(sample_sequence_data, FRESH)
        eid = self._stash(sw, DEAD)
        assert sw._retire_superseded_stash_for_slot("2", "owner@example.com") == 1
        assert eid not in sw._store._list_unclaimed_credentials()

    def test_an_unreadable_slot_drops_nothing(
        self, temp_home, mock_claude_config, sample_sequence_data, monkeypatch
    ):
        """A store that cannot be read must not be read as `no longer holds
        it`. Fails in the same direction the adopt does."""
        sw = self._switcher(sample_sequence_data, FRESH)
        eid = self._stash(sw, DEAD)
        # THE FLAG HAS TO BE THE ONLY THING STOPPING IT. Two earlier shapes
        # each let another guard do the work and stayed green with the flag
        # removed: `("", True)` is caught by `not current`, and returning the
        # STASHED bytes is caught by the fingerprint match. So: readable-
        # looking bytes that DIFFER from the stash, with the flag set.
        monkeypatch.setattr(sw._store, "_read_account_credentials_ex",
