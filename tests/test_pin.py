...
class TestNoModuleDefinesTheSameNameTwice:
    ...
    def test_no_top_level_name_is_defined_twice_in_any_module(self):
        import ast
        import collections
        import pathlib
        ...
        for path in sorted(root.rglob("*.py")):
            ...
            for name, count in names.items():
                if count > 1:
                    offenders.append(f"{path.relative_to(root)}: {name} x{count}")

        assert not offenders, (
            "a module defines the same top-level name more than once — the "
            "later definition silently wins, and which one that is depends on "
            "merge order:\n  " + "\n  ".join(offenders)
        )

    def test_no_class_defines_the_same_method_twice(self):
        """The same shape one level down, minus the property/setter pair.

        `@property` + `@x.setter` legitimately repeat a name, so those are
        excluded by looking at the decorators rather than by naming the three
        pairs `switcher.py` happens to have today.
        """
        import ast
        import collections
        import pathlib
        ...
        def is_accessor(node):
            for dec in node.decorator_list:
                ...
                if text == "property" or text.endswith((".setter", ".deleter",
                                                        ".getter")):
                    return True
            return False
        ...
        for path in sorted(root.rglob("*.py")):
            ...
            for cls in ast.walk(tree):
                ...
                names = collections.Counter(
                    ...
                    and not is_accessor(m)
                )
                for name, count in names.items():
                    if count > 1:
                        offenders.append(
                            f"{path.relative_to(root)}: {cls.name}.{name} x{count}"
                        )

        assert not offenders, (
            "a class defines the same method more than once (property/setter "
            "pairs excluded) — the later one silently wins:\n  "
            + "\n  ".join(offenders)
        )
