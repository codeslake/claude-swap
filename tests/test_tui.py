...
class TestUnswitchableRowsAreListed:
    ...
    def test_the_pinned_account_keeps_its_badge_when_it_has_no_login(self):
        """The pin badge must survive the unswitchable branch.

        A freshly synced machine is exactly where the pinned account is
        likely to have no login yet — the roster travels, credentials do
        not — so that is when you most need to see which row owns the
        claude.ai side.
        """
        from unittest.mock import MagicMock, patch
        from claude_swap.tui.autoview import AutoScreen
        ...
        snap = self._snap(
            self._acct("1", "pinned@x.com", switchable=False),
            self._acct("2", "other@x.com", switchable=True),
        )
        v = AutoScreen.__new__(AutoScreen)
        v._settings = AutoSwitchSettings()
        app = MagicMock()
        ...
        with patch.object(AutoScreen, "app", property(lambda s: app)), \
             patch.object(AutoScreen, "_pinned_email",
                          lambda s: "pinned@x.com"):
            out = str(v._candidates_text(snap, active_number="9"))
        assert "pinned@x.com" in out
        assert "cloud" in out, "the badge must not be dropped with the row"
